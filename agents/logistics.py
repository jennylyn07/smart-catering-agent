...
async def _gpt_interpret_notes(
    ...
    try:
        ...
        system_prompt = (
            ...
            "STAFFING RATIOS (non-negotiable industry standards):\n"
            "- Plated service: 1 server per 10-12 guests. Non-negotiable — required \n"
            "  for synchronized dish drops. Always state the exact number calculated \n"
            "  for this event.\n"
            "- Buffet: 1 server per 20-25 guests\n"
            "- Family style: 1 server per 15 guests\n"
            "- Cocktail reception: 1 server per 20 guests\n"
            "- Full bar service: 1 bartender per 35 guests\n"
            "- Beer and wine only: 1 bartender per 50 guests\n\n"
            "If service style is not explicitly stated in the notes, assume \n"
            "plated service as the default and calculate staffing accordingly, \n"
            "noting the assumption in staffing_notes.\n\n"
            "When staffing_notes are generated, always include the specific \n"
            "calculated number (e.g. \"300 guests ÷ 25 = 12 servers for buffet \n"
            "service\"). Never give a range without also giving the specific number \n"
            "for this event. If multiple service types apply (e.g. buffet food + \n"
            "full bar), calculate and state both separately.\n\n"
            "T-MINUS CRITICAL PATH (use this when generating timeline tasks):\n"
            "- T-0: First bite / service begins (event start time)\n"
            "- T-15min: Final plating and pass-through check\n"
            "- T-2h: Full setup complete, all stations ready\n"
            "- T-6h: Van departure / delivery logistics begin\n"
            "- T-12h: Standard prep start (default if no early setup flag)\n"
            "- T-24h: All ingredients received and checked in\n"
            "- T-48h: Final guest count confirmation\n\n"
            "Reference this CPM when generating extra_timeline_tasks. \n"
            "Tasks should be anchored to T-minus notation where applicable."
        )
